...
void sendlcd(char cmd)
{
	// Rw dat muc thap
	//b2 dat chan rs=0,de noi rang cmd la lenh
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_3, GPIO_PIN_RESET);
	//B3 GUI 8 BIET VAO 8 PIN
	send8bit(cmd);
	//b4 enable cho kytu -- < lcd
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET);
	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_SET);
	HAL_Delay(10);
} 
// ham gui ky tu
void sendchar(char kytu)
{
	// Rw dat muc thap
	//b2 dat chan rs=1
...
int main(void)
{
  ...
  MX_GPIO_Init();
	...
	sendlcd(0x0C);
	//hello
	sendstring("Hello K61 NTU");
	//xuong dong
	sendlcd(0x38);//che do 2 dong
	sendlcd(0xC0);
	sendstring("Ta Long Phi");
